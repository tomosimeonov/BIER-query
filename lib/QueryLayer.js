	...
	parser(sql, function(err, data) {
		if (err) {
			...
			if (data.table) {
				...
				that.selectQuery(data.SELECT, data.FROM[0], data.WHERE, emiter);
...
QueryLayer.prototype.selectQuery = function(properties, namespace, filterPlan, emiter) {
	var that = this;
...
var _createJoinConfiguration = function(plan, context, callback) {
	...
	if (join === undefined || join.cond.operator === undefined || join.cond.operator !== "=")
		...
		builder = builder.setFilterPlan(plan.WHERE).setTimeout(25);

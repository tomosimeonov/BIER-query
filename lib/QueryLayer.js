...
var builderBuilder = require('./QueryConfigurationBuilder');
...
function QueryLayer(storageApis) {
	...
	self.simpleExecutor = simpleExecutorBuilder.QueryExecutor(storageApis,self.statisticHolder);
	self.storageApis.Node.registerMessageHandler(_router({'simple':self.simpleExecutor}));
	return self;
}
...
QueryLayer.prototype.selectQuery = function(properties, namespace, filterPlan, callback) {
	...
	this.simpleExecutor.formatSelectProperties(properties, function(err, formatedSelectProperties) {
		...
		var destinations= [];
		builder = builder.setNamespace(namespace).setDestinations(destinations).setFilterPlan(filterPlan).setFormattedProperties(
				formatedSelectProperties).setTimeout(10);
		that.simpleExecutor.registerNewQuery(builder.buildQueryConfig(), callback, function(err, id) {
			newThat.simpleExecutor.updateWithLocalMatch(id);
			newThat.simpleExecutor.sendQuery(id);
		});
	});

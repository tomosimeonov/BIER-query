...
//Helper method
var decodeInsertStatement = function(context, insert, callback) {
	var findKeyValue = function(keyName, columns, values, callbackFindKeyValue) {
		if (keyName === undefined || columns.indexOf(keyName) == -1)
			callbackFindKeyValue(undefined);
		else
			callbackFindKeyValue(values[columns.indexOf(keyName)]);
	};

	var data = [];
	data.table = insert['INSERT INTO'].table;
	data.columns = insert['INSERT INTO'].columns;
	data.values = insert.VALUES[0];

	context.getPrimaryKeyNameForSchema(data.table, function(err, keyName) {
		if (err)
			keyName = undefined;

		findKeyValue(keyName, data.columns, data.values, function(key) {
			data.key = key;
...
QueryLayer.prototype.createSchema = function(namespace, schema, callback) {
	schemaQueryAPI.permenentSchemaCreateror(schema,function(err,realSchema){
		if(err)
			...
			schemaQueryAPI.isSchemaStructCorrect(realSchema,function(err,correctness){
				if(err)
					callback(err,undefined);
				else{
					//TODO schema in DHT
...
QueryLayer.prototype.insertData = function(namespace, key, columns, values, keepAlive, callback) {
	...
	if (key === undefined || keepAlive === undefined)
		...
		schemaQueryAPI.isSchemaComplied(columns, schema, function(err, schemaComplied) {
			if (schemaComplied) {
				var result = [];
				...
				that.storageApis.put(namespace,key,values,keepAlive,callback);
			} else
...
QueryLayer.prototype.checkQueryPlanAgainstSchema = function(schema, queryPlan, callback) {
	queryPlanProcessor.extractAllProperties(queryPlan, function(data) {
		queryPlanProcessor.isSchemaComplied(data, schema, callback);
	});
};

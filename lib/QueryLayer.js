...
var schemaQueryAPI = require('./SchemaManipulationFuncs');
var insertManipulators = require('./InsertManipulationFuncs');
			...
			if (data.table) {
				...
				that.getPrimaryKeyNameForSchema(data['INSERT INTO'].table, function(err, keyName) {
					if (err || keyName === undefined || keyName === "")
						...
						insertManipulators.decodeInsertStatementPlan(data, keyName, function(err, decodedData) {
							if (err) {
...
QueryLayer.prototype.createSchema = function(namespace, schema, callback) {
	schemaQueryAPI.permenentSchemaCreateror(schema, function(err, realSchema) {
		if (err)
			...
			schemaQueryAPI.isSatisfingSchemaStructure(realSchema, function(err, correctness) {
				if (err)
					callback(err, undefined);
				else {
					// TODO schema in DHT

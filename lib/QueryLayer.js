...
QueryLayer.prototype.executeSQL = function(sql, callback) {
	...
	var simpleSelectCheck = function(data) {
		return data.SELECT !== undefined && !(aggregationCheck(data) || joinCheck(data));
	};

	var aggregationCheck = function(data) {
		return data.SELECT !== undefined && data.SELECT.reduce(function(previousValue, currentValue, index, array) {
	...
	parser(sql, function(err, data) {
		if (err) {
			...
			if (data.table) {
				...
				that.aggregationQuery(data.SELECT, data.FROM[0], data.WHERE, callback);
...
QueryLayer.prototype.selectQuery = function(properties, namespace, filterPlan, callback) {
	...
	//TODO do index search
	
	//TODO execute
	callback(undefined,"Have to do select for " + properties);
};

QueryLayer.prototype.aggregationQuery = function(properties, namespace, filterPlan, callback) {
	callback(undefined,"Have do aggregation for namespace " + namespace);
};

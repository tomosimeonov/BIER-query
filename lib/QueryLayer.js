...
QueryLayer.prototype.selectQuery = function(properties, namespace, filterPlan, dataReceiverEmitter) {
	var that = this;
	...
	this.simpleExecutor.formatSelectProperties(properties, function(err, formatedSelectProperties) {
		...
		that.simpleExecutor.executeQuery(builder.buildQueryConfig(), dataReceiverEmitter);
	});

};

QueryLayer.prototype.joinQuery = function(plan, dataReceiverEmitter) {
	var that = this;
	_createJoinConfiguration(plan, this, function(err, config) {
		if (err) {
			callback(err, undefined);
		} else {
			that.joinExecutor.executeQuery(config, dataReceiverEmitter);
		}
	});

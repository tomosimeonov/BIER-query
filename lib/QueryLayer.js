...
QueryLayer.prototype.executeSQL = function(sql, emiter) {
	...
	parser(sql, function(err, data) {
		if (err) {
			...
			if (data.table) {
				that.createSchema(data.table, data, emiter);
			} else if (data.createIndex) {
				that.createIndex(data.createIndex.table, data.createIndex.property, emiter);
				...
				that.getPrimaryKeyNameForSchema(data['INSERT INTO'].table, function(err, keyName) {
					if (err || keyName === undefined || keyName === "")
						_emitError(emiter, new Error('Problem finding primary key try later'));
					else
				...
				that.selectQuery(data.SELECT, data.FROM[0], data.WHERE,data.TIMEOUT, emiter);
			} else if (joinCheck(data)) {
				that.joinQuery(data, emiter);
			} else
				_emitError(emiter, new Error('Not recognized syntaxis.'));
		}

	});
};

QueryLayer.prototype.createSchema = function(namespace, schema, emiter) {
		...
		if (err)
			...
			schemaFunctions.isSatisfingSchemaStructure(realSchema, function(err, correctness) {
				if (err)
					...
					// TODO schema in DHT
					_emitSuccess(emiter, "Schema for namespace " + namespace + " is been inserted in the database.");
				}
			});
	});
};
...
QueryLayer.prototype.createIndex = function(namespace, property, callback) {
	// TODO create index
	_emitSuccess(emiter, "Have to create index for " + property + " from namespace " + namespace);

};

QueryLayer.prototype.insertData = function(namespace, key, columns, values, keepAlive, emiter) {
	var that = this;
	if (key === undefined || keepAlive === undefined)
		_emitError(emiter, new Error("No primary key value or keep alive provided"));
	else {
		...
		schemaFunctions.isSatisfingSchema(columns, schema, function(err, schemaComplied) {
			if (schemaComplied) {
				...
				that.storageApis.Node.put(namespace, key, values, keepAlive, function(data) {
					_emitSuccess(emiter, data);
				});
			} else
				_emitError(emiter, new Error("Used columns not in the schema"));
		});
	}
};

QueryLayer.prototype.selectQuery = function(properties, namespace, filterPlan,timeout, emiter) {
	var that = this;
	// TODO Extract Schema
	// TODO Validate
	// TODO Index Search
	...
	this.simpleExecutor.formatSelectProperties(properties, function(err, formatedSelectProperties) {
		...
		if (timeout.time !== undefined) {
			builder.setTimeout(timeout.time);
		}
		if (timeout.objects !== undefined) {
...
QueryLayer.prototype.joinQuery = function(plan, emiter) {
	var that = this;
...
var _createJoinConfiguration = function(plan, context, callback) {
	...
	if (join === undefined || join.cond.operator === undefined || join.cond.operator !== "=")
		...
		if (plan.TIMEOUT.time !== undefined) {
			builder.setTimeout(plan.TIMEOUT.time);
		}
		if (plan.TIMEOUT.objects !== undefined) {
...
var _emitError = function(emiter, err) {
	emiter.emit("err", err);
};

var _emitSuccess = function(emiter, success) {

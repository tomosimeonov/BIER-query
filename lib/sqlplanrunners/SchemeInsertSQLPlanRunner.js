...
/**
 * Main entry point for executing SQL statements inserting schemes in the
 * database.
 */
	...
	schemeFunctions.permenentSchemeCreateror(sqlPlan, function(err, realScheme) {
		if (err)
			...
			that.eventHolder.emitEvent(constants.LOG_FINE, [ queryId,
					"Checking if the transformed scheme is satisfing the internal format" ]);
		...
		schemeFunctions.isSatisfingSchemeStructure(realScheme, function(err, correctness) {
			if (err)
				...
				if (correctness) {
					that.eventHolder.emitEvent(constants.LOG_FINER, [ queryId, "Scheme strucure passing tests." ]);
					that.store(namespace, realScheme);
					utilities.emitSuccess(emiter, "Scheme for namespace " + namespace
							+ " has been inserted in the database.");
					utilities.emitQueryFinish(emiter);
				} else {
					that.eventHolder
							.emitEvent(constants.LOG_FINER, [ queryId, "Scheme strucure does not pass tests." ]);

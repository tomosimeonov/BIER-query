...
SchemeInsertSQLPlanRunner.prototype.executePlan = function(sqlPlan, emiter) {
	...
	this.eventHolder.emitEvent("LOG_INFO", [ queryId, "Transforming new scheme into BIER scheme format" ]);
	...
	schemeFunctions.permenentSchemeCreateror(sqlPlan, function(err, realScheme) {
		if (err)
			...
			that.eventHolder.emitEvent("LOG_FINE", [ queryId, "Checking if the transformed scheme is satisfing the internal format" ]);
			schemeFunctions.isSatisfingSchemeStructure(realScheme, function(err, correctness) {
				if (err)
					utilities.emitError(emiter, err);
				else {
					if(correctness){
						that.eventHolder.emitEvent("LOG_FINE", [ queryId, "Scheme strucure passing tests."]);

		...
		if (schema === undefined || schema.primaryKey === "") {
			...
			that.decodeInsertStatementPlan(sqlPlan, keyName, function(err, decodedData) {
				...
				if (err) {
					...
					isSatisfingSchema(decodedData.columns, schema, function(err, schemaComplied) {
						if (schemaComplied) {
							that.areValuesSatisfingSchema(decodedData.columns, decodedData.values, schema, function(
									err, valuesSatisfy) {
								if (valuesSatisfy) {
	...
	if (columns.length == values.length) {
		...
		for (var i = 0; i < columns.length; i++) {
			...
			case "float":
				...
				break;
			default:
				if (typeof values[i] !== schema.properties[columns[i]].type) {
					ans = false;
					notSatisfying(columns[i], callback);
					continue;
				}
		...
		callback(new Error("Number of values does not match number of columns."), false);
	}

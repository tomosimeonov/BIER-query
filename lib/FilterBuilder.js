/**
 * This function creates filter function from the where clause of the provided
 * plan for local data uses. If the query is a join query the two possible
 * objects need to be combined in one and then run through the filter.
 * 
...
var createFilterFromWhereClause = function(whereClause, callback) {
	if (whereClause === undefined || whereClause.length === 0) {
		callback(undefined, function(data) {
			return true;
		});
	} else {

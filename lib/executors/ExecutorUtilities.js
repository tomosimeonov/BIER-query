...
var logMessageReceivedEvent = function(logHolder,message,queryId){
	if (logHolder.isFineLogLevel()) {
		logHolder.emitEvent(logHolder.LOG_FINE,
				"Message received from " + message.origin + " with payload type "
				+ message.payloadType + " for query " + queryId);
	}
	...
	if (logHolder.isFinerLogLevel()) {
		if (message.payloadType === "DATA") {
			logHolder.emitEvent(logHolder.LOG_FINER,"Message received contains data of size " + message.payload.length
					+ " tuples for query with id " + queryId);
		}
		if (message.payloadType === "CONFIG") {
			logHolder.emitEvent(logHolder.LOG_FINER,"Message received is to execute remote query with id " + queryId);
		}
		
		if(message.payloadType === "DATASEARCH") {
			logHolder.emitEvent(logHolder.LOG_FINER,"Message received is to execute local data search only for query with id" + queryId);
		}
		
		if(message.payloadType === "SEC_DATA") {
			logHolder.emitEvent(logHolder.LOG_FINER,"Message received contains data for second table of join query of size " + message.payload.length
					+ " tuples for query with id " + queryId);
		}
	}
	;

...
/**
 * Method to transformStored or Message Delivered data to correct format
 * 
 * @param properties
 *            Dictionary with properties and what type it is.
 * @param data
 *            Array of data tuples
 */
var transformStoredEntryToSchemaValid = function(properties, data, callback) {
...
var logMessageReceivedEvent = function(logHolder, message, queryId) {
	...
	if (message.payloadType === "DATA") {
		logHolder.emitEvent(logHolder.LOG_FINER, [
				trimQueryId(queryId),
				"Message received from " + message.origin + " contains data of size " + message.payload.length
						+ " tuples" ]);
	}
...
var logRemoteErrorOnMissingSchema = function(logHolder, namespace, queryId) {
	logHolder.emitEvent(logHolder.LOG_INFO, [ trimQueryId(queryId), "ERROR: Cannot find schema for " + namespace ]);
};

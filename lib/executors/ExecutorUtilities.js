...
var logMessageReceivedEvent = function(logHolder, message, queryId) {
	...
	logHolder.emitEvent(logHolder.LOG_INFO, "Message received for query " + trimQueryId(queryId));

	if (logHolder.isFineLogLevel()) {
		logHolder.emitEvent(logHolder.LOG_FINE, "Message received from " + message.origin + " for query " + trimQueryId(queryId));
	}
...
var logLocalDataAccessEvent = function(logHolder, namespace, dataSize, queryId) {
	logHolder.emitEvent(logHolder.LOG_INFO, "Local storage accessed for query: " + trimQueryId(queryId));

	if (logHolder.isFineLogLevel()) {
		logHolder.emitEvent(logHolder.LOG_FINE, "Extracting local data for namespace " + namespace + " for query id: " + trimQueryId(queryId));
	}
	if (logHolder.isFinerLogLevel()) {
...
var logDataDeliveredEvent = function(logHolder, dataSize, startTime, queryId) {
	logHolder.emitEvent(logHolder.LOG_INFO, "Data delivered for query: " + trimQueryId(queryId));

	var currentTime = ((new Date()) - startTime) / 1000;
	if (logHolder.isFineLogLevel()) {
		logHolder.emitEvent(logHolder.LOG_FINE, "Data of size " + dataSize + " delivered to requester for query: "
				+ trimQueryId(queryId));
	}
	if (logHolder.isFinerLogLevel()) {
		logHolder.emitEvent(logHolder.LOG_FINER, "Data of size " + dataSize + " delivered " + currentTime
				+ " seconds after initial request to requester for query: " + trimQueryId(queryId));
	}
};

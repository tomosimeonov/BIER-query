...
SimpleExecutor.prototype.executeNonRegQueryOnRemoteCall = function(queryId, configuration, origin) {
	...
	utilities.getscheme(configuration.namespace, this.storageApis, function(err, scheme) {
		if (err) {
			logging.logRemoteErrorOnMissingscheme(that.statisticHolder, configuration.namespace, queryId);
		} else {
			...
			that.queryConfigurations[queryId].primaryKey = scheme.primaryKey;
			that.queryConfigurations[queryId].wasOnlyAggreg = false;

			utilities.produceUnmarshallerDataFunction(scheme, configuration.aggregations, function(unmarshaller) {
...
SimpleExecutor.prototype.registerNewQuery = function(configuration, dataReceiverEmitter, idCallback) {
	...
	utilities.getscheme(configuration.namespace, this.storageApis, function(err, scheme) {
		if (err) {
			...
			// Fixing problem with not correct value in aggregation
			if (that.queryConfigurations[queryId].isAggregation) {
				if (!(configuration.properties.length !== configuration.aggregations.length)) {
					that.queryConfigurations[queryId].wasOnlyAggreg = true;
					that.queryConfigurations[queryId].configuration.properties.push(scheme.primaryKey);
				}
			}
...
SimpleExecutor.prototype.finish = function(queryId) {
	...
	if (this.queryConfigurations[queryId]) {
		...
		if (queryConfiguration.isAggregation && Object.keys(this.runningData[queryId]).length !== 0) {
			...
			aggregationHelper.processData(this.runningData[queryId], queryConfiguration.configuration.properties,
					queryConfiguration.configuration.aggregations, this.nodeId, queryConfiguration.wasOnlyAggreg,
					queryConfiguration.primaryKey, correctCallback);
		} else {

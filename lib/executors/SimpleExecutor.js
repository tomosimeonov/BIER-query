...
SimpleExecutor.prototype.updateQueryHolderInformation = function(queryId, data) {
	if (this.queryConfigurations[queryId] !== undefined) {
		if(this.queryConfigurations[queryId].queryCallback === undefined && !this.queryConfigurations[queryId].isAggregation){
			this.queryConfigurations[queryId].dataReceiverEmitter.emit('data',data);
		}else{
	...
	if (this.queryConfigurations[queryId]) {
		...
		if(correctCallback === undefined) {	
			...
			correctCallback = function(err,data){
				if(data.length !== 0){that.queryConfigurations[queryId].dataReceiverEmitter.emit('data',data);}
				that.queryConfigurations[queryId].dataReceiverEmitter.emit('end');
			};
		}

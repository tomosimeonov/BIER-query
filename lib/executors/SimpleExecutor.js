	...
	utilities.getscheme(configuration.namespace, this.storageApis, function(err, scheme) {
		if (err) {
			...
			that.queryConfigurations[queryId].configuration.timeoutValue = utilities.setTimeouts(queryId, callSender,
					-1, configuration.timeoutValue / 5.0, that);
			that.updateWithLocalMatch(queryId);
		}
	});
	...
	if (this.queryConfigurations[queryId]) {
		...
		if (queryConfiguration.isAggregation && Object.keys(this.runningData[queryId]).length !== 0) {
			...
			aggregationHelper.processData(this.runningData[queryId], queryConfiguration.configuration.properties,
					queryConfiguration.configuration.aggregations, this.nodeId,
					queryConfiguration.wasOnlyAggreg, queryConfiguration.primaryKey,
					correctCallback);
		} else {

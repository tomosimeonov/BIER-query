	...
	this.storageApis.Node.lscan(this.runningQueries[queryId].configuration.namespaceOne, function(localData) {
		if (localData.length !== 0) {
			...
			localDataV.forEach(function(elem) {
				...
				if (that.runningQueries[queryId].tempData = that.runningQueries[queryId].tempData[id]) {
					that.runningQueries[queryId].tempData = that.runningQueries[queryId].tempData[id].push(elem);
				} else {
					that.runningQueries[queryId].tempData = that.runningQueries[queryId].tempData[id] = [ elem ];
				}
			});
...
JoinExecutor.prototype.returnInformation = function(queryId, callback) {
	if (this.runningQueries[queryId]) {
		...
		var realData = this.runningQueries[queryId].data.filter(function(elem) {
			return this.runningQueries[queryId].dataFilter(elem);
		});
...
var _matchingDataSearch = function(queryId, namespace, prop, values, context) {
	...
	var message = messageBuilderBuilder.QueryMessageBuilder().setQueryId(queryId).setJoinType().setOrigin(context.nodeId)

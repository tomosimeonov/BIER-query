 ...
 */
JoinExecutor.prototype.executeQuery = function(configuration, dataReceiverEmitter,idCallback) {
	var that = this;
...
JoinExecutor.prototype.filterDataOnPropValues = function(queryId, namespace, joinProp, values, callback) {
	...
	this.storageApis.Node.lscan(namespace, function(localData) {
		...
		var localDataV = objectKeys.map(function(key) {
...
JoinExecutor.prototype.executeNonRegQueryOnRemoteCall = function(queryId, configuration, origin) {
	...
	this.queryConfigurations[queryId].configuration.timeoutValue = utilities.setTimeouts(queryId, callSender, -1,
			configuration.timeoutValue / 3, this);
...
JoinExecutor.prototype.updateQueryHolderInformation = function(queryId, data) {
	...
	if (this.queryConfigurations[queryId] !== undefined) {
		utilities.filterData(queryId, data, this.queryConfigurations[queryId].dataFilter, function(queryId, data) {
	...
	if (this.queryConfigurations[queryId]) {
		...
		data.forEach(function(elem) {
			...
			for (var i = 0; i < others.length; i++) {
				realData.push(_combine(that.queryConfigurations[queryId].configuration.namespaceTwo, elem,
						that.queryConfigurations[queryId].configuration.namespaceOne, others[i]));
			}
			if (both) {
				realData.push(_combine(that.queryConfigurations[queryId].configuration.namespaceTwo, elem, undefined,
						undefined));
			}
		});
...
JoinExecutor.prototype.finish = function(queryId) {
	if (this.queryConfigurations[queryId]) {
		...
		if (this.queryConfigurations[queryId].queryCallback) {
			...
			utilities.logDataDeliveredEvent(this.statisticHolder, this.runningData[queryId].length,

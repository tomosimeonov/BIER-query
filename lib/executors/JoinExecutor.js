...
JoinExecutor.prototype.message = function(message) {
	...
	if (message.origin === this.nodeId) {
		...
		if (this.queryConfigurations[queryId] === undefined) {
			if (message.payloadType === "DATA") {
				...
				if (message.payload.datasearch !== undefined) {
					var senderReturn = function(data) {
						if (data.length !== 0) {
							var newMessage = messageBuilderBuilder.QueryMessageBuilder().setQueryId(queryId)
									.setJoinType().setOrigin(message.origin).setCustomPayloadType("SEC_DATA")
									.setPayload(data).buildMessage();
							that.storageApis.Node.send(origin, JSON.stringify(newMessage), function(data) {
							});
						}
					};

					this.filterDataOnPropValues(message.queryId, message.payload.namespace, message.payload.prop,
			...
			switch (message.payloadType) {
			case "DATA":
				if (this.queryConfigurations[queryId].state === STORE) {
					this.updateQueryHolderInformation(queryId, message.payload);
				} else if (this.queryConfigurations[queryId].state === DISCARD) {
					this.storageApis.send(message.origin, message, function() {
					});
				}
				break;
...
JoinExecutor.prototype.updateQueryHolderInformation = function(queryId, data) {
	if (this.queryConfigurations[queryId] !== undefined) {
		...
		if (this.queryConfigurations[queryId].queryCallback === undefined) {
			this.queryConfigurations[queryId].dataReceiverEmitter.emit('data', realData);
		} else {
			this.runningData[queryId] = this.runningData[queryId].concat(realData);
		}
		...
		if (this.queryConfigurations[queryId].currentNumObjects === this.queryConfigurations[queryId].configuration.maxObjects) {
			...
			utilities.objectCountStopper(queryId, this.queryConfigurations[queryId].currentNumObjects, this);
		}
	}
};
/**
...
JoinExecutor.prototype.updateQueryHolderInformationWithSecondData = function(queryId, data) {
	if (this.queryConfigurations[queryId]) {
		...
		var filter = this.queryConfigurations[queryId].dataFilter;
		realData = realData.filter(function(elem) {
			return filter(elem);
		});
		...
		if (this.queryConfigurations[queryId].currentNumObjects === this.queryConfigurations[queryId].configuration.maxObjects) {
			// Have to terminate
			this.finish(queryId, this.queryConfigurations[queryId].queryCallback);
		} else if (this.queryConfigurations[queryId].configuration.maxObjects !== -1) {
			// Restart timer for object count
			utilities.objectCountStopper(queryId, this.queryConfigurations[queryId].currentNumObjects, this);
		}
...
JoinExecutor.prototype.updateWithLocalMatch = function(queryId) {
	...
	this.storageApis.Node
			.lscan(
					this.queryConfigurations[queryId].configuration.namespaceOne,
					function(localData) {
						if (localData.length !== 0) {
							var values = [];
							var localDataV = Object
									.keys(localData)
									.map(
											function(key) {
												values
														.push(localData[key][that.queryConfigurations[queryId].configuration.joinPropertyOne]);
												return localData[key];
											});
							...
							if (that.queryConfigurations[queryId].configuration.type !== 0) {
								var properData = localDataV.map(function(elem) {
									_combine(that.queryConfigurations[queryId].configuration.namespaceOne, elem,
											undefined, undefined);
								});
							...
							that.filterDataOnPropValues(queryId,
									that.queryConfigurations[queryId].configuration.namespaceTwo,
									that.queryConfigurations[queryId].configuration.joinPropertyTwo, values, function(
											data) {

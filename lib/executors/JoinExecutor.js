		...
		data
				.forEach(function(elem) {
					...
					for (var i = 0; i < others.length; i++) {
						that.queryConfigurations[queryId].currentNumObjects = that.queryConfigurations[queryId].currentNumObjects + 1;
						that.runningData[queryId].push(_combine(
								that.queryConfigurations[queryId].configuration.namespaceOne, elem,
								that.queryConfigurations[queryId].configuration.namespaceTwo, others[i]));
					}
					if (both) {
						that.queryConfigurations[queryId].currentNumObjects = that.queryConfigurations[queryId].currentNumObjects + 1;
						that.runningData[queryId].push(_combine(
								this.queryConfigurations[queryId].configuration.namespaceOne, elem, undefined,
								undefined));
					}
				});
		if (this.queryConfigurations[queryId].currentNumObjects === this.queryConfigurations[queryId].configuration.maxObjects) {
...
JoinExecutor.prototype.returnInformation = function(queryId, callback) {
	if (this.queryConfigurations[queryId]) {
		...
		var realData = this.runningData[queryId].filter(function(elem) {
			return filter(elem);
		});

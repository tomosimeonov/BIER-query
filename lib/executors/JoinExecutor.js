...
var _combine = function(namespaceA, elemA, namespaceB, elemB, properties) {
	...
	if (elemA)
		for (key in elemA) {
			if (properties.size === 0 || (properties[namespaceA] && properties[namespaceA].indexOf(key)!==-1)) {
	...
	if (elemB)
		for (key in elemB) {
			...
			if (properties.size === 0 || (properties[namespaceB] && properties[namespaceB].indexOf(key)!==-1))
				data[id] = elemB[key];
		}
	return data;
};

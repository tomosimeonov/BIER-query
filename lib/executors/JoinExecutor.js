...
JoinExecutor.prototype.message = function(message) {
	...
	if (message.payloadType === constants.PAYLOAD_TYPE_DATA_SEARCH) {
		...
		if (this.queryConfigurations[queryId] !== undefined
				&& this.queryConfigurations[queryId].state === constants.STORE) {
	...
	var senderReturn = function(data) {
		if (data.length !== 0) {			
			var messageB = messageBuilderBuilder.QueryMessageBuilder().setQueryId(queryId).setJoinType().setOrigin(
					message.origin).setCustomPayloadType(constants.SEC_DATA);
			logging.logDataDeliveredEventJoinSecTable(context.statisticHolder, Object.keys(data).length, queryId);
...
var processMessageContainingDataNonOriginEnd = function(queryId, message, context) {
	switch (message.payloadType) {
	case constants.DATA_MESSAGE:
		if (context.queryConfigurations[queryId].state === constants.STORE) {
		...
		} else if (context.queryConfigurations[queryId].state === constants.DISCARD) {
...
JoinExecutor.prototype.distribute = function(queryId) {
	logging.logDistributingConfigurationEvent(this.statisticHolder, [], queryId);
		...
		for ( var key in data) {
			...
			if (shouldKeppOnlyThisToo) {
				realData[key] = _combine(that.queryConfigurations[queryId].configuration.namespaceTwo, data[key],
						undefined, undefined, that.queryConfigurations[queryId].configuration.properties);
			}
			...
			for ( var secondKey in others) {
				realData["" + key + "-" + secondKey] = _combine(
						...
						that.queryConfigurations[queryId].configuration.properties);
			}
...
// Checks for projections
var _combine = function(namespaceA, elemA, namespaceB, elemB, properties) {
	var data = {};
	var hasProjection = Object.keys(properties).length !== 0;
	if (elemA)
		for (key in elemA) {
			if (!hasProjection || (properties[namespaceA] && properties[namespaceA].indexOf(key) !== -1)) {
	...
	if (elemB)
		for (key in elemB) {
			...
			if (!hasProjection || (properties[namespaceB] && properties[namespaceB].indexOf(key) !== -1))
				data[id] = elemB[key];
		}
	return data;
};

// Extract both schemas from the database
...
var _matchingDataSearch = function(queryId, namespace, prop, values, context) {
	...
	var message = messageBuilderBuilder.QueryMessageBuilder().setQueryId(queryId).setJoinType().setOrigin(
			context.nodeId).setCustomPayloadType(constants.PAYLOAD_TYPE_DATA_SEARCH).setPayload(payload).buildMessage();

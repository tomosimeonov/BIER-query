...
JoinExecutor.prototype.updateQueryHolderInformation = function(queryId, data) {
	if (this.runningQueries[queryId]) {
		...
		data.forEach(function(elem) {
			var others = this.runningQueries[queryId].tempData[elem[joinP]];
			for (var i = 0; i < others.length; i++) {
				this.runningQueries[queryId].currentNumObjects = this.runningQueries[queryId].currentNumObjects + 1;
				this.runningQueries[queryId].data.push(_combine(
						this.runningQueries[queryId].configuration.namespaceOne, elem,
						this.runningQueries[queryId].configuration.namespaceOTwo, others[i]));
			}
	...
	this.storageApis.Node.lscan(this.runningQueries[queryId].configuration.namespaceOne, function(localData) {
		if (localData.length !== 0) {
			...
			localdataV.forEach(function(elem) {
				var id = elem[this.runningQueries[queryId].configuration.joinPropertyOne];
				if (tempLocalData[id]) {
					tempLocalData[id].push(elem);
				} else {
					tempLocalData[id] = [ elem ];
				}
			});

			if (this.runningQueries[queryId].configuration.type !== 0) {
				var properData = localDataV.map(function(elem) {
					_combine(this.runningQueries[queryId].configuration.namespaceOne, elem, undefined, undefined);
				});
				this.runningQueries[queryId].currentNumObjects = this.runningQueries[queryId].currentNumObjects
						+ properData.length;
				that.runningQueries[queryId].data = that.runningQueries[queryId].data.concat(properData);
			}

			that.runningQueries[queryId].tempData = that.runningQueries[queryId].tempData.concat(tempLocalData);

			_matchingDataSearch(queryId, this.runningQueries[queryId].configuration.namespaceTwo,
					this.runningQueries[queryId].configuration.joinPropertyTwo, values, this);
		}
		;
	});
};
...
JoinExecutor.prototype.returnInformation = function(queryId, callback) {
	if (this.runningQueries[queryId]) {
		...
		// TODO FILTER DATA
		callback(undefined, queryData.data);
	}
};
...
var _matchingDataSearch = function(queryId, namespace, prop, values, context) {
	...
	var message = messageBuilderBuilder.QueryMessageBuilder().setQueryId(queryId).setJoinType().setOrigin(this.nodeId)
			.setPayloadTypeConfig().setPayload(payload).buildMessage();

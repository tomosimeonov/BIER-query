...
var insertPropertyIntoHolder = function(property, propertiesHolder) {
	if (property.indexOf('.') > -1) {
		...
		var prop = property.slice(property.indexOf('(')+1, property.length - 1);
		updateHolder('default', prop, propertiesHolder);
...
var extractAllProperties = function(plan, callback) {
	...
	var combineWithPropertiesHolder = function(array) {
		for ( var prop in array) {
			if (propertiesHolder[prop] === undefined)
				...
				for(var i = 0; i < array[prop].length;i++){
					if(propertiesHolder[prop].indexOf(array[prop][i]) === -1){
						propertiesHolder[prop].push(array[prop][i]);
					}
				}
		}
	};
	...
	var whereFunction = function(data) {
		...
		if(propertiesHolder['default'] && Object.keys(propertiesHolder).length === 1){
			propertiesHolder[namespace] = propertiesHolder['default'];
			delete propertiesHolder['default'];
		}else if(Object.keys(propertiesHolder).length >= 3){
			callback(new Error("Wrong syntaxis in SELECT part!."),undefined);
		}else{}
		callback(undefined,propertiesHolder);
	};

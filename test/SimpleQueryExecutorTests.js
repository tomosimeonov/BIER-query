...
exports.simpleTests = {
	shouldNotReturnDataOnNoDataPassingFilter : function(test) {
		...
		queryExecutor.registerQuery(undefined, 1, function(err,data) {
	...
	shouldreturnDataOnDataPassingFilter : function(test) {
		...
		queryExecutor.registerQuery(undefined, 1, function(err,data) {
			test.equal(1, data.length, 'Should return one.');
	...
	shouldReturnDataOnObjectCountStopper : function(test) {
		...
		queryExecutor.registerQuery(2, undefined, function(err,data) {
			test.equal(2, data.length, 'Should return one.');
